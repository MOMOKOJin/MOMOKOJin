...
# step 3 move simultaneously
# Part1 main_extend_v1.py

# based on the original code, we can remove the Outermost condition judgment ,
# and after that,two players will finish one-step-action in one game turn , to 
# allow players to move simultaneously. ps: the computer player's action is 
# based on player B ,with 4 direction by 'jikl' ,and I set one dictionary
# connect 4 words with 1,2,3,4 ,then I create one lambda with a randint(1,4), 
# every turn it will create one random number form 1 to 4 , and with each
# number with one corresponding direction , which is the move_position of 
# computer player but because of the random of the number ,at beginning 
# step there are 50% possibility to trigger the game over condition ,so it 
# is a little stupid . There are much room for improvement. 
